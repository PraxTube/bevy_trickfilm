...
impl AnimationPlayer2D {
    ...
    pub fn start(&mut self, handle: Handle<AnimationClip2D>) -> &mut Self {
        self.start_from_elapsed(handle, 0.0, 0.0)
    }
    ...
    /// Start playing an animation from the current elapsed state. This should only be called on
    /// animations that have the same duration.
    pub fn play_continue(&mut self, handle: Handle<AnimationClip2D>) -> &mut Self {
        if self.animation.animation_clip != handle || self.paused() {
            self.start_from_elapsed(handle, self.animation.elapsed, self.animation.seek_time);
        }
        self
    }
